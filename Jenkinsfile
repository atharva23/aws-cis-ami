pipeline {
  ...
  stages {
    stage('Build AMI') {
      steps {
        sh '''
          latest_ami_date=$(aws ec2 describe-images --region "us-east-1" -filters "Name=name,Values=ami-*" "Name=state,Values=available" "Name=architecture,Values=x86_64" --query 'reverse(sort_by(Images, &CreationDate))[0].CreationDate' --output text)
          current_date=$(date +%s)
